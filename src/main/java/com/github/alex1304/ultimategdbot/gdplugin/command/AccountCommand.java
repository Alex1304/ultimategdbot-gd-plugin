...
public class AccountCommand {
	...
	public Mono<Void> run(Context ctx) {
		return ctx.bot().database().findByID(GDLinkedUsers.class, ctx.author().getId().asLong())
	...
	public Mono<Void> runLink(Context ctx, GDUser gdUsername) {
		...
		return ctx.bot().database().findByID(GDLinkedUsers.class, authorId)
	...
	private Mono<Void> handleDone(Context ctx, GDLinkedUsers linkedUser, GDUser user, GDUser botUser) {
		return ctx.reply("Checking messages, please wait...")
				.flatMap(waitMessage -> gdService.getGdClient().getPrivateMessages(0)
						...
						.then(ctx.bot().database().save(linkedUser))
