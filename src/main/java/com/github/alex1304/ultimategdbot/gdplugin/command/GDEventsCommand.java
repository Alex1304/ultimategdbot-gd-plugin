...
@CommandDescriptor(
		aliases = "gdevents",
		shortDescription = "tr:strings.gd/gdevents_desc"
)
...
public class GDEventsCommand {
	...
	@CommandDoc("tr:strings.gd/gdevents_run_dispatch")
	public Mono<Void> runDispatch(Context ctx, String eventName, @Nullable Long levelId) {
		...
		switch (eventName) {
			...
			default:
				if (levelId == null) {
					return Mono.error(new CommandFailedException(ctx.translate("strings.gd", "error_id_not_specified")));
				}
				switch (eventName) {
					...
					case "awarded_level_updated":
						...
						break;
					default:
						return Mono.error(new CommandFailedException(ctx.translate("strings.gd", "error_id_not_specified", ctx.prefixUsed())));
				}
		}
	...
	@CommandAction("loop")
	@CommandDoc("tr:strings.gd/gdevents_run_loop")
	public Mono<Void> runLoop(Context ctx, String action) {
		switch (action) {
			case "start":
				return Mono.fromRunnable(gdService.getGdEventscannerLoop()::start)
						.then(ctx.reply(ctx.translate("strings.gd", "event_loop_started")))
						.then();
			case "stop":
				return Mono.fromRunnable(gdService.getGdEventscannerLoop()::stop)
						.then(ctx.reply(ctx.translate("strings.gd", "event_loop_stopped")))
						.then();
			default:
				return Mono.error(new CommandFailedException(
						ctx.translate("strings.gd", "error_unknown_action", ctx.prefixUsed())));
		}
	}
	
	@CommandAction("dispatch_all_awarded_resuming_from")
	@CommandDoc("tr:strings.gd/gdevents_run_dispatch_all_awarded_resuming_from")
	@FlagDoc(
			@FlagInfo(name = "max-page", valueFormat = "number", description = "tr:strings.gd/gdevents_flag_max_page")
	)
	...
	private static MessageSpecTemplate paginateEvents(Translator tr, int page, int lastPage, List<AwardedLevelAddedEvent> events) {
		...
		return new MessageSpecTemplate(tr.translate("strings.gd", "dispatch_list") + "\n\n"
				+ tr.translate("strings.common", "pagination_page_count", page + 1, lastPage + 1) + '\n'
				+ events.stream()
						.skip(page * 10)
						.limit(10)
