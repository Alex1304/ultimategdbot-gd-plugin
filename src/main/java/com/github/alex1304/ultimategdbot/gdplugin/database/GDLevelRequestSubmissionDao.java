...
public interface GDLevelRequestSubmissionDao {
	...
	@SqlQuery("SELECT * FROM " + TABLE + " "
			+ "NATURAL JOIN " + GDLevelRequestReviewDao.TABLE + " "
			+ "WHERE submission_id = ?")
	@UseRowReducer(SubmissionWithReviewReducer.class)
	Optional<GDLevelRequestSubmissionData> get(long id);
	
	@SqlQuery("SELECT * FROM " + TABLE + " WHERE guild_id = ? AND is_reviewed = 0 ORDER BY submission_timestamp")
	List<GDLevelRequestSubmissionData> getQueuedSubmissionsInGuild(long guildId);

	@SqlQuery("SELECT * FROM " + TABLE + " WHERE is_reviewed = 0")
	...
	@SqlUpdate("INSERT INTO " + TABLE + " VALUES (NULL, :levelId, :youtubeLink, NULL, "
			+ "NULL, :guildId, :submitterId, :submissionTimestamp, :isReviewed)")
	@GetGeneratedKeys
	long insert(@BindPojo GDLevelRequestSubmissionData data);
	
	@SqlUpdate("UPDATE " + TABLE + " SET message_channel_id = :channelId, channel_id = :messageId "
			+ "WHERE submission_id = :submissionId")
	void setMessageAndChannel(long submissionId, long channelId, long messageId);
	
	@SqlUpdate("UPDATE " + TABLE + " SET message_channel_id = :channelId, message_id = :messageId, isReviewed = 1"
			+ "WHERE submission_id = :submissionId")
