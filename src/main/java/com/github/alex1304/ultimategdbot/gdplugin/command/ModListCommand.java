...
@CommandDescriptor(
		aliases = "modlist",
		shortDescription = "tr:strings.gd/modlist_desc"
)
	...
	public Mono<Void> run(Context ctx) {
		return Mono.zip(ctx.bot().service(EmojiService.class).emoji("mod"), ctx.bot().service(EmojiService.class).emoji("elder_mod"), 
				...
				.flatMap(function((modEmoji, elderModEmoji, modList) -> {
					var sb = new StringBuilder("**__" + ctx.translate("strings.gd", "mod_list") + "__\n**");
					sb.append(ctx.translate("strings.gd", "modlist_intro", ctx.prefixUsed()) + "\n\n");
